...
def get_devices(adb_path='adb'):
    ...
    with open(os.devnull, 'wb') as devnull:
        subprocess.check_call([adb_path, 'start-server'], stdout=devnull,
                              stderr=devnull)
    out = split_lines(subprocess.check_output([adb_path, 'devices']).strip().decode('utf-8'))
...
def _get_device_by_type(flag, adb_path):
    ...
    try:
        serial = subprocess.check_output(
            [adb_path, flag, 'get-serialno']).strip().decode('utf-8')
...
def _subprocess_check_output(*args, **kwargs):
    try:
        ...
        raise subprocess.CalledProcessError(ex.returncode, args[0],
                                            output=ex.output)
    ...
    def shell_popen(self, cmd, kill_atexit=True, preexec_fn=None,
        ...
        if os.name == 'nt':
            ...
            if preexec_fn is None:
                ...
                preexec_fn = _wrapper
        _p = _subprocess_Popen(command, creationflags=creationflags,
                               preexec_fn=preexec_fn, **kwargs)
        if kill_atexit:
            atexit.register(_p.kill)
        return _p
